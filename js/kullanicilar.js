...
window.kullaniciyiKaydet=async function(){
  ...
  try{
    if(!mUid){
      ...
      await sb.from('kullanicilar').insert({id:sd?.user?.id||uid(),ad,soyad,email,kullanici_adi:kAdi,rol,varsayilan_kasa_id:varsayilanKasa||null});
      bil(`${ad} ${soyad} eklendi ✓`);
    }else{
      await sb.from('kullanicilar').update({ad,soyad,email,kullanici_adi:kAdi,rol,varsayilan_kasa_id:varsayilanKasa||null}).eq('id',mUid);
      bil('Güncellendi ✓');
    }
  ...
  document.getElementById('kullanici-liste').innerHTML=kullanicilar.map(k=>{
    ...
    return `<div class="kullanici-kart">
      ...
      <div class="kk-bilgi">
        ...
        ${varsKasa?`<span style="font-size:10px;color:var(--yazi3);margin-left:6px">💵 ${varsKasa.ad}</span>`:''}
      </div>
